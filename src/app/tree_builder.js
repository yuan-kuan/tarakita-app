...
const generateFirstKey = R.curry((lens, treeState) =>
  R.pipe(R.view(lens), convertToParentId, insertFirstOrder)(treeState)
);

const generateNextKey = R.curry((lens, treeState) =>
  R.pipe(R.view(lens), incrementOrder)(treeState)
);
...
const generateDoc = R.curry((id, value, type) =>
  R.pipe(R.set(L.id, id), R.set(L.value, value), R.set(L.type, type))({})
);
...
const storeState = (treeState) =>
  free.of(treeState).map(removeInternals).map(R.values).chain(bulkDocs);
